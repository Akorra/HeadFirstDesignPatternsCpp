...
public:
  ...
  static std::unique_ptr<GumballMachine> create(std::string location, int numberGumballs);
...
private:
  GumballMachine(std::string location, int numberGumballs) : _count(numberGumballs), _location(location) {}
...
public:
  GumballMonitor(GumballMachine& machine) : _machine(machine) {}
  void report() const;
};
